//! NVSE plugin for Fallout New Vegas crash capture.
//!
//! This crate provides the Rust side of a hybrid C++/Rust NVSE plugin.
//! The C++ layer handles NVSE registration and VEH setup, while Rust
...
mod ffi {
    ...
    /// Plugin information from the game's data handler.
    #[derive(Debug, Clone)]
    struct PluginInfo {
        /// Plugin filename (e.g., "FalloutNV.esm").
        name: String,
        /// Load order index.
        index: u8,
    }
    ...
    extern "Rust" {
        ...
        /// Called when the game has finished loading data.
        fn on_data_loaded();
    ...
    unsafe extern "C++" {
        ...
        /// Get the current load order from the data handler.
        fn get_load_order() -> Vec<PluginInfo>;

        /// Get the game version.
        fn get_game_version() -> String;

        /// Get the NVSE version string.
        fn get_nvse_version() -> String;
    }
}
